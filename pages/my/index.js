...
Page({
    ...
    if (userInfo) {
      const { avatarUrl, nickname } = JSON.parse(userInfo)
      this.setData({
        apiUserInfoMap: {
  ...
  updateUserInfo(e) {
    ...
    // const SDKVersion = Number(appBaseInfo.SDKVersion.replace(/\./g, ""))
    wx.getUserProfile({
      lang: 'zh_CN',
      desc: '用于完善会员资料',
      success: res => {
  ...
  changeUser() {
    console.log(1)
    wx.navigateTo({
      url: '/pages/my/change-user?userInfo=' + JSON.stringify(this.data.apiUserInfoMap)
    })
  },
  // 获取用户手机号
  getPhoneNumber (e) {
